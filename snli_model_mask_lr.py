...
class SNLIModel(object):
  ...
  def __init__(self, is_training, config):
    ...
    self.batch_triple = tf.placeholder(tf.int32, [self.config.batch_size,self.config.xmaxlen*self.config.ymaxlen])
    ...
    with tf.device("/cpu:0"):
      ...
      triple_embedding_matrix=np.load("../data/glove/testrelation.npy")
      triple_embedding = tf.Variable(triple_embedding_matrix,trainable=True, name="triple_embedding")
      
      input_xemb = tf.nn.embedding_lookup(embedding, self.x)
      input_yemb = tf.nn.embedding_lookup(embedding,self.y)
      
      triple_emb=tf.nn.embedding_lookup(triple_embedding,self.batch_triple)#(b,x_len*y_len,relemb)
      if is_training and config.keep_prob < 1:
        input_xemb = tf.nn.dropout(input_xemb, config.keep_prob)
        input_yemb = tf.nn.dropout(input_yemb, config.keep_prob)
        triple_emb = tf.nn.dropout(triple_emb, config.keep_prob)
     

    with tf.variable_scope("update_x") as scope:
      input_xemb=self.update_input(input_xemb,triple_emb)
  ...
  def update_input(self,input_emb,triple_emb):
    triple_emb=tf.reshape(triple_emb,[self.config.xmaxlen,self.batch_size,self.config.ymaxlen,self.config.relation_size])
    emb_list=[]
    for i in range(self.config.xmaxlen):
      with tf.variable_scope("triple_fnn") as scope:
        if i > 0:
          scope.reuse_variables()
        triple = tf.reshape(triple_emb[i],[self.batch_size,self.config.ymaxlen*self.config.relation_size])#(b,y,50)->(b,y*50)
        triple_fnn=self.fnn(tf.cast(triple,tf.float32),self.config.relation_size)#(b,50)
        emb_list.append(triple_fnn)
    emb_list=tf.convert_to_tensor(emb_list)
    rel_emb=tf.reshape(emb_list,[self.batch_size,self.config.xmaxlen,self.config.relation_size])#(b,x_len,50)
    input_upemb=tf.concat([rel_emb,input_emb],axis=-1)
    return input_upemb
