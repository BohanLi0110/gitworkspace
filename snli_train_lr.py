...
def fill_placeholder(data, model,config):
  batch_x,batch_y,batch_label,batch_x_mask,batch_y_mask, batch_x_len,batch_y_len,batch_triple= data.next_batch(config.batch_size)
  feed_dict = {model.x:batch_x , 
                ...
                model.batch_triple:batch_triple
                }
  ...
  with tf.Graph().as_default():
    ...
    with sv.managed_session() as session:
      ...
      for i in range(config.MAXITER):
        start_time=time.time()
        #train_acc,train_loss,train_global_step,learning_rate,triple_emb = run_epoch(session,data=Train, model=m,config=config, eval_op=m.optim, verbose=True)
